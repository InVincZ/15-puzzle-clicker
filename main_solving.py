...
if __name__ == "__main__":
    ...
    starting_position, puzzle = Find_position.main()
    tests = [
        puzzle
        ]
    print('\n--------------------------')
    ...
    for p in tests:
        path, moves, cost, num_eval = slide_solver.solve(p, is_goal, 250)
        # slide_print(p)
        print('\nSOLUTION: (' + str(len(moves)) + ' Moves in total)\n')
        print(", ".join({-1: "Left", 1: "Right", -4: "Up", 4: "Down"}[move[1]] for move in moves))

        print('\n Solving puzzle...\n')
        output = ", ".join({-1: "Left", 1: "Right", -4: "Up", 4: "Down"}[move[1]] for move in moves)

        mousehandler.main(starting_position, output)
        print('DONE!')
        # print(cost, num_eval)
